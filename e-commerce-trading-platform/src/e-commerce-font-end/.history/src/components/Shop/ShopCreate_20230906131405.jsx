...
const ShopCreate = () => {
  ...
  //       "http://localhost:8080/trading-platform/store/", shopData);
  ...
  useEffect(() => {
    // Lấy thông tin cửa hàng của người dùng khi component được tải
    axios.get('http://localhost:8080/api/store/')
      .then((response) => {
        // Kiểm tra nếu có thông tin cửa hàng
  ...
  const createNewStore = () => {
    // Gửi yêu cầu POST để tạo cửa hàng mới
    axios.post('http://localhost:8080/api/store/', storeData)
      .then((response) => {
        if (response.status === 200) {
          // Xử lý thành công (thông tin cửa hàng mới được tạo)
          console.log('Cửa hàng mới đã được tạo:', response.data);
        } else {
          // Xử lý lỗi nếu cần
        }
      })
          ...
          <Row>
            {loading ? (
              ...
              <Col lg="6" className="m-auto text-center">
                ...
                <Form className="auth__form" onSubmit={handleInputChange}>
