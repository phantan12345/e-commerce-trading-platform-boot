...
public interface MessagesRepository extends JpaRepository<Messages, Integer> {
    ...
    @Query(value = "SELECT "
            ...
            nativeQuery = true)
    List<Object[]> getUserDistinctMessages(@Param("userId") Integer userId, Pageable pageable);

    @Query("SELECT m  FROM Messages m WHERE (m.sentBy = :loggedInUserId AND m.sentTo = :chatRecipientId) OR (m.sentBy = :chatRecipientId AND m.sentTo = :loggedInUserId) ORDER BY m.id DESC")
    List<Messages> getUserMessagesWithUser(@Param("loggedInUserId") User loggedInUserId, @Param("chatRecipientId") User chatRecipientId, Pageable paginationObject);
